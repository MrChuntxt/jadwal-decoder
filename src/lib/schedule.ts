...
export type Session = RawRow & { id: string; dateISO: string; dateLabel: string; course: string; time: string; start: string; end: string; roomText: string; roomUrl: string; room: string; raw: string; lecturerName: string; lecturerTitle: string; lecturerUrl: string; matchStatus: MatchStatus; matchNote?: string };
...
export function makeSession(raw:RawRow, rawStr:string=""):Session { const date=nextDate(raw.hari), rm=room(raw.ruang), mt=mergeTime(raw.waktu), parts=mt.split("-"); const start=parts[0]||"", end=parts.length>1?parts[1]:""; return {...raw,id:uid(),issues:[...(raw.issues||[])],dateISO:date.iso,dateLabel:date.label,course:courseName(raw.mataKuliah),time:mt,start,end,roomText:rm.text,roomUrl:rm.url,room:rm.text||raw.ruang,raw:rawStr,lecturerName:raw.dosen,lecturerTitle:"",lecturerUrl:manualStaffUrl(raw.dosen),matchStatus:"pending"}; }
export function recalc(s:Session,field:EditableRawKey,value:string):Session { const n:Session={...s,[field]:value,issues:(s.issues||[]).filter(x=>norm(x)!==norm(field))}; if(field==="kelas")return n; if(field==="hari"){const d=nextDate(value);return {...n,dateISO:d.iso,dateLabel:d.label};} if(field==="mataKuliah")return {...n,course:courseName(value)}; if(field==="waktu"){const mt=mergeTime(value),parts=mt.split("-");return {...n,time:mt,start:parts[0]||"",end:parts.length>1?parts[1]:""};} if(field==="ruang"){const r=room(value);return {...n,roomText:r.text,roomUrl:r.url,room:r.text||value};} if(field==="dosen")return {...n,lecturerName:value,lecturerTitle:"",lecturerUrl:manualStaffUrl(value),matchStatus:"manual",matchNote:"Edited — run lecturer lookup again."}; return n; }
